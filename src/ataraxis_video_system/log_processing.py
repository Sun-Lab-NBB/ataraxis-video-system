...
def extract_logged_camera_timestamps(
    ...
    *,
    ...
    if n_workers == 1 or reader.message_count < _PARALLEL_PROCESSING_THRESHOLD:
        return tuple(message.timestamp_us for message in reader.iter_messages() if message.payload.size == 0)
...
def _extract_unique_components(paths: list[Path] | tuple[Path, ...]) -> tuple[str, ...]:
    ...
    for path in paths_list:
        ...
        for component in components:
            ...
            is_unique = all(component not in other_path.parts for other_path in paths_list if other_path != path)
...
def _process_frame_message_batch(
    ...
    return [message.timestamp_us for message in reader.iter_messages(keys=keys) if message.payload.size == 0]
