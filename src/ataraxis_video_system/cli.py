...
from .pipeline import process_logs  # pragma: no cover
...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True, file_okay=False, dir_okay=True, readable=True, path_type=Path),
    help="The path to the directory containing the .npz log archives to process.",
)
@click.option(
    "-od",
    "--output-directory",
...
@click.option(
    ...
    type=str,
    multiple=True,
    help="Source log ID to process. Repeat to specify multiple IDs. If not provided, all discovered "
...
@click.option(
    ...
    default=False,
    show_default=True,
    help="Determines whether to extract camera frame timestamps from the log archives.",
)
@click.option(
    "-w",
    "--workers",
...
def process(
    ...
    *,
    ...
    """Processes VideoSystem log archives to extract frame timestamps and other data.

    Functions as the entry point for processing the data stored in the .npz log archives generated by VideoSystem
    instances during runtime. If no specific job flags or job identifiers are provided, the pipeline processes all
    available log files.
    """
    process_logs(
