...
class ArenaVideoSystem:
    ...
    def __init__(
        ...
        self.exp_array = global_arrays[
            0
        ]  # This is the general experiment status array used throughout the project to control quit states
        self.ttl_array = global_arrays[
            1
        ]  # This array is used to communicate the state of incoming frames (when they are received) to an amc process that bounces these ttl signals to the recording system
        self.save_frame_array = global_arrays[
            2
        ]  # This array is used to control when the grabbed frames should be written to disk vs simply displayed to the user
    ...
    def data_grabber_module(self):
        ...
        try:
            h.add_file(
                file_path=self.grabber_backend,
                check_existence=True,
                check_validity=True,
            )  # Imports general interface cti file to enable harvester to find and work with the camera
        except FileNotFoundError:
            sys.exit(
                f"Warning! Error encountered in {sys.argv[0]} script at line {get_caller_line_number()} for {self.name} video system. GenTL Backend file {self.grabber_backend} does not exist. Execution aborted!"
            )
        except OSError:
            sys.exit(
                f"Warning! Error encountered in {sys.argv[0]} script at line {get_caller_line_number()} for {self.name} video system. GenTL Backend file {self.grabber_backend} is not a valid GenTL backend. Execution aborted!"
            )
        ...
        grabber.num_buffers = (
            self.num_buffers
        )  # Specifies the number of buffers to be used by the system. Only used when async_grabber is enabled. If the code runs without issue, you may not need many of these, they are mostly helpful if the processing halts for a while. In this case, the buffers do just what their name implies and prevent data loss

        # Connects to shared memory arrays
        shm_1, save_frame_arr = axu.connect_shm_arr(
            self.save_frame_array
        )  # This array is used to control when the grabbed frames should be written to disk vs simply displayed to the user
        shm_2, frame_ttl = axu.connect_shm_arr(
            self.ttl_array
        )  # This array is used to communicate when a frame has been acquired. When that happens, a dedicated amc sends a ttl pulse over to recording system to synchronize all ttl signals to the same clock
        shm_3, exp_state = axu.connect_shm_arr(
            self.exp_array
        ...
        grabber.remote_device.node_map.AcquisitionFrameRate.value = (
            self.fps
        )  # Uses GenTL map to pass acquisition fps value to the camera. NOTE, make sure your camera supports the requested fps
        ...
        if self.async_grabber:
            grabber.start(
                run_as_thread=True
            )  # Starts grabbing incoming camera frames. Uses a separate thread to do so for improved concurrency
        else:
            grabber.start(
                run_as_thread=False
        ...
        while (exp_state[0] != 255) | (grabber.num_holding_filled_buffers > 0):
                ...
                if display_limiter != 1:
                    if (self.debug == 1) | (self.verbose == 1):
                        axu.print_string(
                            ...
                            string=f"{self.name} frame saving: initialized",
                            pad=0,
                        )
                    display_limiter = 1
            ...
            # Alternatively, if frame_saving trigger is set to 0 and save_frames is enabled, disables frame saving
                ...
                if display_limiter != 2:
                    if (self.debug == 1) | (self.verbose == 1):
                        axu.print_string(
                            log_path=self.log_path,
                            string=f"{self.name} frame saving: stopped",
                            pad=0,
                        )
                    display_limiter = 2
            ...
            )  # Reshapes grabbed 1D array into 2D frame array. Also uses full copy operation to retrieve it before the buffer gets invalidated (and content-wiped) by re-queueing
    ...
    def image_recording_module(self, img_queue):
        ...
        while True:
            ...
            try:
                tff.imwrite(
                    im_path, data=cam_frame
                )  # Uses default zlib and compression level 0 (uncompressed) setting of tifffile library to save incoming frames as .tif files
            except TypeError:
                pass
    ...
    def save_config(self):
        ...
        with open(config_path, "ab") as fl:
            pic.dump(
                self.info_config, protocol=pic.HIGHEST_PROTOCOL, file=fl
            )  # Serializes and saves the configs as pickle file
    ...
    def initialize_avs(avs, debug=0, verbose=1, log_path=None):
        ...
        if (verbose == 1) | (debug == 1):
            axu.print_string(
                log_path=log_path,
                string=f"AVS {avs.name} grabber process: initialized",
                pad=1,
            )
        ...
        frame_array = (
            avs.data_grabber_module()
        )  # Triggers image acquisition. This returns a list of frames with their timestamps

        if (verbose == 1) | (debug == 1):
            axu.print_string(
                log_path=log_path,
                string=f"AVS {avs.name} grabber module shutdown: complete. Exporting frame_list and configs...",
                pad=0,
            )
        ...
        if (verbose == 1) | (debug == 1):
            axu.print_string(
                log_path=log_path,
                string=f"AVS {avs.name} asynchronous execution: complete",
                pad=0,
            )
        ...
        for i, video_id in enumerate(converter_params.keys()):
            ...
            if not os.path.exists(video_file):
                ...
                if (verbose == 1) | (debug == 1):
                    axu.print_string(
                        log_path,
                        f"Encoding video {i + 1} out of {encoding_runs} requested for session {description_data[0]} out of {description_data[1]} total",
                        0,
                    )
                ...
                process = subprocess.Popen(
                    ...
                    shell=True,
                )  # Note, the current implementation prints standard ffmpeg progress data to the terminal window which enables crude progress tracking
                ...
                if (verbose == 1) | (debug == 1):
                    axu.print_string(
                        log_path,
                        f"Video {i + 1} out of {encoding_runs} requested for session {description_data[0]}: Encoded. FFMPEG output ticket:",
                        1,
                    )
    ...
    def compress_images(
        ...
        input_args = (
            []
        )  # A mega-list where input arguments for the compression module are packaged as lists (so a list of lists)
        for image in image_list:
            ...
            image_split = image_string.split(
                "."
            )  # Splits around the comma. Should result in the whole part + extension. Selecting index 0 in the resultant array returns the whole path part without extension
            ...
            input_args.append(
                [image, output_image, compression, compressed_arr, failed_arr]
            )  # path to input, path to output, desired compression level, compressed and failed shm arrays
        ...
        processing_pool = mp.Pool(
            axu.get_max_mp_processes()
        )  # Sets it to num logical cpus, so physical * 2 unless you are on the intel train
        ...
        while not compression_result.ready():
            ...
            if (
                ...
                if (verbose == 1) | (debug == 1):
                    axu.print_string(
                        log_path,
                        f"Compressing... {complete_percent} % complete. {processed_files[0]} files out of {len(image_list)} processed. {failed_files[0]} file compressions failed. Elapsed time: {elapsed_current} minutes",
                        0,
                    )
        ...
        if (verbose == 1) | (debug == 1):
            axu.print_string(
                log_path,
                f"Compression: complete. Compressed {len(image_list)} files, which took {round(delta_t / 60, 2)} minutes",
                1,
            )
        ...
        if failed_files[0] > 0:
            ...
            if (verbose == 1) | (debug == 1):
                axu.print_string(
                    log_path,
                    f"However, {failed_arr[0]} files failed to compress. The files were:",
                    0,
                )
    ...
    def image_compression_module(
        ...
        return [
            image_name,
            image_status,
        ]  # returns the image name and its processing status
