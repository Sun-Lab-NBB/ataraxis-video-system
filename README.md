...
if __name__ == "__main__":
    ...
    # pipeline writes results as Feather files in the specified output directory.
    ...
    # Reads the processed timestamps from the output Feather file and computes the camera frame rate.
    dataframe = pl.read_ipc(source=processed_directory / "camera_101_timestamps.feather")
...
DataFrame column in Feather output files) that matches the order in which the frames were saved to disk as an .mp4
file. Each timestamp is given as the number of microseconds elapsed since the UTC epoch onset.
...
manage job lifecycle (scheduled, running, succeeded, or failed).
...
This library provides an MCP server that exposes camera discovery, configuration, video recording, and log data
processing functionality for AI agent integration.
